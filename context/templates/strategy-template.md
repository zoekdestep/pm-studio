# [Compelling Title]
## [Subtitle that frames the insight or question]

**Author:** [Name] (alias) | **Last modified:** [Date] | [Optional: tools/collaborators that helped]

---

## Introduction

[2-3 paragraphs setting up the context. What's the current situation? What's the problem or opportunity? Why does this matter now?]

### Purpose of this document

[1-2 sentences on what this doc does and doesn't do. Is it a playbook? An exploration? A recommendation?]

---

## [Framework Section Title]

[Introduction to the framework or research backing your argument]

**Implication:** [Key takeaway from this section]

---

## [Key Insight/Lever 1]

[Heading that names the concept]

[Explanation of the concept with evidence]

**Example:** [Concrete example showing this in action]

---

## [Key Insight/Lever 2]

[Explanation with evidence]

**Example:** [Concrete example]

---

## [Key Insight/Lever 3]

[Explanation with evidence]

**Example:** [Concrete example]

---

## [Key Insight/Lever 4]

[Explanation with evidence]

**Example:** [Concrete example]

---

## Principles Based on These [Insights/Levers]

| [Category] | Principle |
|------------|-----------|
| [Insight 1] | [Actionable principle derived from the insight] |
| [Insight 2] | [Actionable principle derived from the insight] |
| [Insight 3] | [Actionable principle derived from the insight] |
| [Insight 4] | [Actionable principle derived from the insight] |

---

## [Additional Context Section - e.g., Personalization, Adjacent Considerations]

[Any additional framing that affects how the principles apply]

---

## Applying These Principles to [Specific Problem]

[Introduction to how the framework applies to your specific situation]

### Examples

[Concrete examples showing the principles applied]

| [Category] | [Application] |
|------------|---------------|
| [Type 1] | [Example of principles applied] |
| [Type 2] | [Example of principles applied] |
| [Type 3] | [Example of principles applied] |

---

## Follow-ups and Next Steps

[What should happen next based on this thinking?]

### [Proposed approach - e.g., Rapid experimentation]

[Description of recommended next steps]

Example [experiments/approaches] can include:

1. [Option 1 with rationale]
2. [Option 2 with rationale]
3. [Option 3 with rationale]

---

## Open Questions and Risks

[The most notable open questions and risks]

### [Risk 1]

[Description of the risk and why it matters. How might we mitigate it?]

### [Risk 2]

[Description of the risk]

---

## Sources

1. [Author]. ([Year]). [Title]. [Publication]. [Link if available]
2. [Author]. ([Year]). [Title]. [Publication].
3. [Continue numbered list...]
